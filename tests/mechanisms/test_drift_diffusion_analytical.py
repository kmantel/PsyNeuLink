...
def check_drift_diffusion_analytical(B, data, degenerate_cases=False):
    ...
    for i in range(NUM_CHECKS):
        ...
        if degenerate_cases:
            np.testing.assert_allclose(results_b[1:6], ground_truth[0:5], atol=1e-10, equal_nan=True)
        else:
            np.testing.assert_allclose(results_b[1:], ground_truth, atol=1e-10, equal_nan=True)
