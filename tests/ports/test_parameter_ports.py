...
class TestParameterPorts:
    ...
    @pytest.mark.parametrize(
        'mech_type, mech_kwargs, param_name', [
            *[(pnl.ExternalMemoryMechanism, {'field_type': pnl.FieldType.KEY, 'field_shape': (1, 1), 'field_memory': [[[0], [0]]]}, p) for p in ['matrix', 'decay_rate']],
        ]
    )
    def test_created_after_function(self, mech_type, mech_kwargs, param_name):
        m = mech_type(**mech_kwargs)
        assert getattr(m.parameters, param_name) in m.parameter_ports
