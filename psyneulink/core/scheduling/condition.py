...
specified `TimeScale` or `Time <_Scheduler_Absolute_Time>`):
...
__all__ = [
    ...
    'AtRun', 'AfterRun', 'AfterNRuns', 'AfterTimeStep', 'AfterTrial', 'All', 'AllHaveRun', 'Always', 'And', 'Any',
...
def _parse_absolute_unit(n, unit):
    try:
    ...
    if n is not None and not isinstance(n, pnl._unit_registry.Quantity):
        n = n * unit
...
class TimeInterval(AbsoluteCondition):
    ...
    Attributes:
        ...
        unit
            the `pint.Unit` to use for scalar values of *n*, *start*,
            and *end*
    ...
    Satisfied when:
        ...
        Every *n* units of time at/after *start* and before/through
        *end*
    ...
    Notes:
        ...
        **x**, terminating at any time before **x**. If in doubt, use
        ...
        If the scheduler is not set to `exact_time_mode = True`, *start_inclusive* and *end_inclusive* may not behave as expected. See `Scheduler_Exact_Time` for more info.
    """
    def __init__(
        self,
        n: typing.Union[int, str, pint.Quantity] = None,
        ...
        if n is start is end is None:
            raise ConditionError('At least one of n, start, end must be specified')

        if start is not None and end is not None and start > end:
            raise ConditionError(f'Start ({start}) is later than {end}')

        n = _parse_absolute_unit(n, unit)
        ...
        def func(scheduler, execution_id):
            ...
            if n is not None:
                satisfied &= (clock.time.absolute - offset) % n == 0
    ...
    @property
    def absolute_intervals(self):
        return [self.n] if self.n is not None else []
