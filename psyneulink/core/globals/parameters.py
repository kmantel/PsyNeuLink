...
from psyneulink.core.globals.utilities import call_with_pruned_args, copy_iterable_with_shared, \
    get_alias_property_getter, get_alias_property_setter, get_deepcopy_with_shared, unproxy_weakproxy, create_union_set, safe_equals, get_function_default_value
...
def get_init_signature_default_value(obj, parameter):
    ...
    if '__init__' in obj.__dict__:
        return get_function_default_value(obj.__init__, parameter)
    else:
    ...
    def reset(self):
        ...
        try:
            ...
            if self._owner._param_is_specified_in_class(self.name):
                try:
                    val = cls_param_value.default_value
                except AttributeError:
                    val = cls_param_value

                # TODO: replace this when allowing None as a proper value
                if val is not None:
                    self.default_value = val
                    return

        # check for default in __init__ signature
        init_param_value = get_init_signature_default_value(self._owner._owner, self.name)
        if init_param_value != inspect._empty and init_param_value is not None:
            self.default_value = init_param_value
            return
