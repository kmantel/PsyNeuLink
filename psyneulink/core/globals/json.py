...
The below is a Stroop model with conflict monitoring, and its output
...
it corresponds to a ParameterState. In this case, it will indicate the \
`ParameterState` in a `source<>` field:
...
function). Modeling-specific entries are populated for additional \
specification based on the environment.
...
their names, that are part of the Compositions
    ...
    if isinstance(value, list):
        ...
        if (
            ...
            try:
                ...
                raise PNLJSONError(
                    f'Invalid python type specified in JSON object: {value_type}'
                ) from e
        ...
        if match is not None:
            comp_identifer, state_type, name = match.groups()
...
def _generate_component_string(
    ...
    for arg, val in sorted(parameters.items(), key=lambda p: p[0]):
        try:
            ...
            # Examples:
            #   ControlMechanism.control_signals_spec / control_signals
...
def _generate_condition_string(condition_dict, component_identifiers):
    def _parse_condition_arg_value(value):
        ...
        if value not in component_identifiers and pnl_str is not None:
...
def generate_script_from_json(model_input):
    """
        ...
        `general JSON format <_JSON_Model_Specification>`
