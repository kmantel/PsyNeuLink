...
class Composition(Composition_Base, metaclass=ComponentsMeta):
    ...
    def add_linear_learning_pathway(self,
        ...
        for learnable_projection in [lp for lp in learning_pathway.learning_components[LEARNED_PROJECTIONS]
            ...
            mech_lr_specified = learning_mech.parameters.learning_rate._user_specified
            proj_lr_specified = learnable_projection.parameters.learning_rate._user_specified

            # only set if one is specified and the other is not
            if mech_lr_specified != proj_lr_specified:
                if mech_lr_specified:
                    # otherwise assign LearningMechanism's learning rate or default to Projection
                    learning_mech_lr = learning_mech.parameters.learning_rate._get(context)
                    _lr = (
                        learning_mech_lr
                        if (learning_mech_lr is not None and learning_mech_lr is not True)
                        else learning_rate
                    )
                    learnable_projection.parameters.learning_rate.set(_lr, context)
                elif proj_lr_specified:
                    proj_lr = learnable_projection.parameters.learning_rate._get(context)
