...
from psyneulink.core.compositions.visualization import _animate_execution, AnimationSettings, show_graph
...
from psyneulink.core.globals.keywords import \
    ...
    MODEL_SPEC_ID_COMPOSITION, MODEL_SPEC_ID_NODES, MODEL_SPEC_ID_PROJECTIONS, \
    ...
    PARAMETER, PARAMETER_CIM_NAME, PROCESSING_PATHWAY, PROJECTION, PULSE_CLAMP, SAMPLE, SHADOW_INPUT_NAME, SHADOW_INPUTS, SOFT_CLAMP, SSE, \
    TARGET, TARGET_MECHANISM, VARIABLE, WEIGHT, INITIAL_FRAME, EXECUTION_SET
...
class Composition(Composition_Base, metaclass=ComponentsMeta):
    ...
    def evaluate(
        ...
        if self._animate is not False and self._animate.simulations is not False:
    ...
    def run(
        ...
        if animate is not False:
            try:
                animate = AnimationSettings(composition_name=self.name, **animate)
            except TypeError:
                raise CompositionError(
                    "{} argument for {} method of {} ({}) must be a boolean or "
                    "a dictionary of argument specifications for its {} method".format(
                        repr('animate'), repr('run'), self.name, self._animate, repr('show_graph')
                    )
                )
        ...
        if self._animate is not False:
            ...
            movie_path = self._animate.movie_dir + '/' + self._animate.movie_name
            self._animation[0].save(fp=movie_path,
                                    format='GIF',
                                    save_all=True,
                                    append_images=self._animation[1:],
                                    duration=self._animate.duration * 1000,
                                    loop=0)
            print(f"\nSaved movie for '{self.name}' in '{self._animate.movie_name}'")
            if self._animate.show:
    ...
    def execute(
        ...
        if self._animate is not False:
            ...
            try:
                _animate_execution(self, INITIAL_FRAME, context)
            except KeyError:
        ...
        if self._animate is not False and self._animate.show_cim:
            _animate_execution(self, self.input_CIM, context)
        # FIX: END
        ...
        if (self.enable_controller and
            ...
            if (
                ...
                context.add_flag(ContextFlags.CONTROL)
                if self._animate != False and self._animate.show_controller:
                    _animate_execution(self, self.controller, context)
                context.remove_flag(ContextFlags.CONTROL)
        ...
        for next_execution_set in execution_scheduler.run(termination_conds=termination_processing,
            ...
            if self._animate is not False and self._animate.unit == EXECUTION_SET:
                _animate_execution(self, next_execution_set, context)
            ...
            for node in next_execution_set:
                ...
                # ANIMATE node ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                if self._animate is not False and self._animate.unit == COMPONENT:
                    _animate_execution(self, node, context)
        ...
        if self._animate is not False and self._animate.show_cim:
            self._animate_execution(self, self.output_CIM, context)
        # FIX: END
        ...
        if (self.enable_controller and
            ...
            if (
                ...
                if self._animate is not False and self._animate.show_controller:
                    self._animate_execution(self, self.controller, context)
    ...
    def show_graph(self, *args, **kwargs):
        """
            See `show_graph` and `Visualizing a Composition
            <Composition_Visualization>`_
        """
        show_graph(self, *args, **kwargs)
